(function () {
  ...
  const fmt = v => RF.trimFixed(v, 4), dbm = v => RF.formatDbm(v) + ' dBm';
  ...
  function writeQuery() {
    ...
    ['source-power','bandwidth','bandwidth-unit','source-temp'].forEach(id => q.set(id, $(id).value));
  ...
  function renderStages() {
    $('stages').innerHTML = stages.map((s, i) => `<section class="stage" data-index="${i}" aria-label="Stage ${i + 1}">
      ...
      <label>${s.kind === 'passive' ? 'Loss (positive dB)' : 'Gain (dB)'}<input data-key="db" inputmode="decimal" value="${escape(s.db)}" aria-label="Stage ${i + 1} ${s.kind === 'passive' ? 'loss' : 'gain'}"></label>
  ...
  function compute() {
    ...
    const parsed = stages.map(s => ({ kind: s.kind, db: RF.parseNumber(s.db), nf: RF.parseNumber(s.nf),
    ...
    $('power-rows').innerHTML = result.rows.map((r, i) => `<tr class="${r.headroom !== null && r.headroom < 0 ? 'over-limit' : ''}"><td>${escape(stages[i].name || `Stage ${i + 1}`)}</td><td>${fmt(r.gainDb)} dB</td><td>${dbm(r.outputDbm)}</td><td>${parsed[i].limit === null ? '—' : dbm(parsed[i].limit)}</td><td>${r.headroom === null ? 'Unspecified' : fmt(r.headroom) + ' dB'}</td></tr>`).join('');
    $('noise-metrics').innerHTML = metric('Total gain', fmt(result.gainDb) + ' dB') + metric('Cascaded noise figure', fmt(result.nf) + ' dB') +
      metric('Equivalent input noise temp.', fmt(result.equivalentTemperature) + ' K') + metric('Input source noise', dbm(result.inputNoiseDbm)) +
      metric('Output noise', dbm(result.noiseDbm)) + metric('Output SNR', fmt(result.snr) + ' dB');
    $('noise-rows').innerHTML = result.rows.map((r, i) => `<tr><td>${escape(stages[i].name || `Stage ${i + 1}`)}</td><td>${fmt(r.stageNf)} dB</td><td>${fmt(r.nf)} dB</td><td>${dbm(r.noiseDbm)}</td></tr>`).join('');
    ...
    const lines = ['Matched stages at one frequency, small-signal CW power; gains and noise figures are constant over equivalent noise bandwidth.',
      `Source = ${power} dBm; bandwidth B = ${bandwidth} Hz; Tsource = ${temperature} K; T₀ = 290 K.`];
    let previousGain = 1;
    result.rows.forEach((r,i) => {
      const s = parsed[i], f = 10 ** (r.stageNf / 10), g = 10 ** (r.gainDb / 10);
      lines.push('', `Stage ${i + 1}: ${stages[i].name}`, `Power out = ${power} + (${fmt(r.cumulativeGainDb)}) = ${fmt(r.outputDbm)} dBm`,
        s.kind === 'passive' ? `Fstage = 1 + (10^(${s.db}/10) − 1) × ${s.temperature}/290 = ${fmt(f)}` : `Fstage = 10^(${s.nf}/10) = ${fmt(f)}`,
        `Gain = 10^(${fmt(r.gainDb)}/10) = ${fmt(g)}; prior gain = ${fmt(previousGain)}`,
        `Added input noise factor = (${fmt(f)} − 1)/${fmt(previousGain)} = ${fmt(r.contribution)}`);
      if (r.headroom !== null) lines.push(`Headroom = ${s.limit} − (${fmt(r.outputDbm)}) = ${fmt(r.headroom)} dB`);
      previousGain *= g;
    });
    lines.push('', `Ftotal = 1 + Σ added factors = ${fmt(result.factor)}; NF = 10 log₁₀(Ftotal) = ${fmt(result.nf)} dB`,
      `Teq = (Ftotal − 1) × 290 = ${fmt(result.equivalentTemperature)} K`,
      `Noise output = k × B × (Tsource + Teq) × Gtotal`,
      `= 1.380649e−23 × ${bandwidth} × (${temperature} + ${fmt(result.equivalentTemperature)}) × 10^(${fmt(result.gainDb)}/10) W = ${dbm(result.noiseDbm)}`,
      `Output SNR = ${fmt(result.outputDbm)} − (${fmt(result.noiseDbm)}) = ${fmt(result.snr)} dB`);
  ...
  $('bandwidth-unit').addEventListener('change', () => {
    ...
    if (Number.isFinite(value)) $('bandwidth').value = String(Number((value * scales[bandwidthUnit] / scales[$('bandwidth-unit').value]).toPrecision(12)));
    bandwidthUnit = $('bandwidth-unit').value; loadError = ''; compute();
  });
  $('copy-link').addEventListener('click', () => { if (result) Bench.copy(location.href); });
  $('copy-result').addEventListener('click', () => { if (result) Bench.copy(`CW power chain | Source ${dbm(n('source-power'))}\n` + result.rows.map((r,i) => `${stages[i].name}: ${dbm(r.outputDbm)}; limit ${stages[i].limit || 'unspecified'}; headroom ${r.headroom === null ? 'unspecified' : fmt(r.headroom) + ' dB'}`).join('\n') + `\nGain ${fmt(result.gainDb)} dB | NF ${fmt(result.nf)} dB | B ${n('bandwidth')} ${$('bandwidth-unit').value} | Tsource ${n('source-temp')} K | Output noise ${dbm(result.noiseDbm)} | SNR ${fmt(result.snr)} dB`); });
