...
them, and name their end blocks as the terminations the parameter is referenced to rather
than as the analyzer, since after port-impedance conversion those are the device's impedances. Every diagram is drawn by one builder as HTML blocks joined by buses, so VOPP, Gain and
